...
# Adapted from: https://github.com/microsoft/Megatron-DeepSpeed/blob/8822a5ced6ce74d926fbe0f49cdca6bb3389bef8/megatron/arguments.py
...
def parse_args(extra_args_provider=None, ignore_unknown_args=False):
    ...
    # Args from environment, launched with torchrun
    if args.world_size is None:
        args.rank = int(os.getenv('RANK', '0'))
        args.world_size = int(os.getenv("WORLD_SIZE", '1'))
        args.master_ip = os.getenv("MASTER_ADDR")
        args.master_port = os.getenv("MASTER_PORT")
...
def validate_args(args, defaults={}):
    ...
    # comment that, since SAILOR allows training with diff tmp
    # assert args.world_size % args.tensor_model_parallel_size == 0, 'world size'\
    #     ' ({}) is not divisible by tensor model parallel size ({})'.format(
    #         args.world_size, args.tensor_model_parallel_size)
    # Zero bubble pipeline is defined on deepspeed's scheduler
    if args.max_tensor_model_parallel_size==0:
        args.max_tensor_model_parallel_size = args.tensor_model_parallel_size
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--remove-final-layer-norm', action='store_true',
                       help='Remove final layer norm'),
    group.add_argument('--num-transformer-layers-original', type=int, default=None, required=True,
                       help='Number of transformer layers in the original model (we use 1 layer for profiling)')
    group.add_argument('--window_size', type=int, default=None, required=False,
                       help='window size (required from GPT-Neo)')
    return parser
...
def _add_regularization_args(parser):
    ...
    group.add_argument('--residual-dropout', type=float, default=0.1,
                       help='Residual dropout')
...
def _add_training_args(parser):
    ...
    group.add_argument('--model-name', type=str, required=True, help='Model Name (for profiling)')
    group.add_argument('--gpu-type', type=str, required=True, help='GPU type (for profiling)')
    group.add_argument('--results-dir', type=str, required=True, help='Results_dir (for profiling)')
    group.add_argument('--sailor-profile', action='store_true', help='Profile or not, sailor-style')
    group.add_argument('--oobleck-profile', action='store_true', help='Oobleck-specific mem-only profiling')
    group.add_argument('--galvatron-profile', action='store_true', help='Galvatron-specific profiling')
    group.add_argument('--varuna-profile', action='store_true', help='Varuna-specific profiling')
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--max-tensor-model-parallel-size', type=int, default=0,
                       help='Maximum Degree of tensor model parallelism. Just used for padding reasons')
    ...
    group.add_argument('--rank', type=int, default=None, help='Global rank')
    group.add_argument('--world_size', type=int, default=None, help='World size')
    group.add_argument('--master_ip', type=str, default=None, help='MASTER IP')
    group.add_argument('--master_port', type=str, default=None, help='MASTER PORT')
    group.add_argument('--distributed-config-file', type=str, default=None, help='Config file with ranks and grouping per stage')
