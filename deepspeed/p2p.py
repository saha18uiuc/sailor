...
def send(tensor, dest_stage, async_op=False):
    ...
    dest_ranks = []
    if _grid._topo.all_pipeline_parallel_group_ranks is None:
        dest_rank = _grid.stage_to_global(stage_id=dest_stage)
        dest_ranks.append(dest_rank)
    else:
        myrank = torch.distributed.get_rank()
        for pipe_group in _grid._topo.all_pipeline_parallel_group_ranks:
            if myrank in pipe_group:
                dest_rank = pipe_group[dest_stage]
                dest_ranks.append(dest_rank)
    ...
    for dest_rank in dest_ranks:
        if async_op:
            ...
            dist.send(tensor, dest_rank)


def send_with_group_idx(tensor, dest_stage, dest_idx, async_op=False):
    ...
    if _grid._topo.all_pipeline_parallel_group_ranks is None:
        ...
        for pipe_group in _grid._topo.all_pipeline_parallel_group_ranks:
            if myrank in pipe_group:
                dest_ranks.append(pipe_group[dest_stage])
        dest_rank = dest_ranks[dest_idx]
...
def recv_with_group_idx(tensor, src_stage, src_idx, async_op=False):
    ...
    if _grid._topo.all_pipeline_parallel_group_ranks is None:
        ...
        for pipe_group in _grid._topo.all_pipeline_parallel_group_ranks:
            if myrank in pipe_group:
                src_ranks.append(pipe_group[src_stage])
