...
def reconstruct_tensor(partial_tensor_list, group):
    ...
    start_all = None
    end_all = 0
    for obj in partial_tensor_list:
        if isinstance(obj, tuple):
            inputs_obj = tuple(t.clone() for t in obj)
        else:
            inputs_obj = obj.clone()

        meta = inputs_obj[0].to('cpu')
        meta = meta[1+meta[0]:]
        partition = meta[2:]
        src_rank = meta[1]
        buf = inputs_obj[1]
        start = partition[src_rank]
        end = partition[src_rank+1]
        flat_tensor[start:end].copy_(buf)

        if start_all is None:
            start_all = start.item()
        end_all = max(end_all, end.item())

    all_tmp_ranks = torch.distributed.get_process_group_ranks(group)
    my_global_rank = torch.distributed.get_rank()
    tp_rank = all_tmp_ranks.index(my_global_rank)

    if group_size > 1:
        num_parts = dist.get_world_size(group=group)
        psize = num_el // group_size
        start = tp_rank * psize
        end = (tp_rank+1) * psize
        local_data = flat_tensor[start:end]
        num_parts = dist.get_world_size(group=group)
        dist.all_gather_into_tensor(flat_tensor, local_data, group=group)

    full_tensor = flat_tensor.view(tensor_shape).clone().detach()
    return full_tensor

class PartitionedTensor:

    def __init__(self, tensor, group=None, partition_meta=None):
        super().__init__()
    ...
    @classmethod
    def from_meta(cls, meta, world_size, local_part, group=None, device=get_accelerator().device_name()):
        assert meta.dtype == torch.long
        dummy = torch.ones(world_size)
