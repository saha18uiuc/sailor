...
class PipelineModule(nn.Module):
    ...
    Args:
        ...
        layer_partitioning (list[int], optional): If not None, it overrides the partition_method for layer partitioning
    ...
    def __init__(self,
        ...
        self._partition_layers(method=partition_method, layer_partitioning=layer_partitioning)
    ...
    def _partition_layers(self, method='uniform', layer_partitioning=None):
        num_stages = self._topo.get_dim('pipe')
        stage_id = self._topo.get_coord(self.global_rank).pipe

        if self.global_rank == 0:
            if layer_partitioning:
                logger.info(f'Layer partitioning given: {layer_partitioning}')
            else:
                logger.info(f'Partitioning pipeline stages with method {method}')

        if layer_partitioning:
            self.parts = layer_partitioning
        else:
            method = method.lower()
        ...
        print(f"self.parts is {self.parts}")
