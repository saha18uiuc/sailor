...
class ProcessTopology:
    ...
    def __init__(self, axes, dims, input_file=None):
        ...
        if input_file is None:
            ...
            self.all_data_parallel_group_ranks = None
            self.all_pipeline_parallel_group_ranks = None
            self.all_model_parallel_group_ranks = None
        else:
            # topology given as a file
            with open(input_file, 'r') as f:
                config = json.load(f)
            # we know the coordinates are 'pipe', 'data', 'model'
            for i,stage_config in enumerate(config):
                for j,node in enumerate(stage_config):
                    for k, global_rank in enumerate(node):
                        key = {'pipe': i, 'data': j, 'model': k}
                        key = self.ProcessCoord(**key)
                        self.mapping[key] = global_rank
            self.get_all_groups(config, dims[0], dims[1])


    def get_all_groups(self, config, pp, dp):
        tps_stage = [set() for _ in range(pp)] # set of the different tp for that stage
        ...
        for stage_config in config:
            for node in stage_config:
                self.all_model_parallel_group_ranks.append(node)

        self.all_data_parallel_group_ranks = []
        self.all_data_parallel_group_ranks_lists = []
        for j in range(pp):
            ...
            for i in range(num_dp_groups_j):
                ...
                self.all_data_parallel_group_ranks.append(group_i)
                self.all_data_parallel_group_ranks_lists.append(group_i_list)


        self.all_pipeline_parallel_group_ranks = []
        self.all_pipeline_parallel_group_ranks_lists = []
        tps_pp_stage = [set() for _ in range(dp)] # set of the different tp for that stage
        for i in range(dp):
            ...
            for j in range(num_pp_groups_i):
                ...
                self.all_pipeline_parallel_group_ranks.append(group_j)
                self.all_pipeline_parallel_group_ranks_lists.append(group_j_list)
...
class PipeHeterogeneousTopology(ProcessTopology):
    """ A topology for hybrid pipeline, model, and data parallelism. """

    def __init__(self, num_pp, num_mp, num_dp, input_file):
        #super().__init__(axes=['pipe', 'data', 'model'], dims=[num_pp, num_dp, num_mp])
        super().__init__(axes=['pipe', 'data', 'model'], dims=[num_pp, num_dp, num_mp], input_file=input_file)
...
class PipelineParallelGrid:
    ...
    def __init__(self, topology=None, process_group=None):
        ...
        self.model_parallel_size = max(self._topo.get_dim('model'), 1) # TODO: check  this
        ...
        self.dp_group_list = []
        self.dp_proc_group_list = []
        self.dp_groups = self._topo.all_data_parallel_group_ranks
        ...
        for g in self.dp_groups:
            ...
            if self.global_rank in g:
                self.dp_group_list.append(g)
                self.dp_proc_group_list.append(proc_group)
        ...
        self.pp_group_list = []
        self.pp_proc_group_list = []
        self.pipe_groups = self._topo.all_pipeline_parallel_group_ranks
        if self.pipe_groups is None:
            self.pipe_groups = self._topo.get_axis_comm_lists('pipe')
        for ranks in self.pipe_groups:
            proc_group = dist.new_group(ranks=ranks)
            if self.global_rank in ranks:
                self.pp_group_list.append(ranks)
                self.pp_proc_group_list.append(proc_group)
        assert self.pp_proc_group_list is not None

        # # Create new ProcessGroup for model (tensor-slicing) collectives

        self.mp_group = []
        self.slice_rank = -1
        self.model_groups = self._topo.all_model_parallel_group_ranks
        ...
        for g in self.model_groups:
            ...
            if self.global_rank in g:
                self.slice_group = g
                self.slice_group_world_size = len(g)
                self.slice_rank = g.index(self.global_rank)
    ...
    def _build_p2p_groups(self):
        ...
        parallel stages.
        """
        comm_lists = self._topo.all_pipeline_parallel_group_ranks
        if comm_lists is None:
    ...
    def _is_grid_valid(self):
        ranks = []
        for p,rank in self._topo.mapping.items():
            ranks.append(rank)
        ranks = sorted(ranks)
        expected_ranks = list(range(dist.get_world_size()))
    ...
    def get_pipe_parallel_group(self):
        ...
        return self.pp_proc_group_list[0]
    ...
    def get_data_parallel_group(self):
        ...
        return self.dp_proc_group_list
    ...
    def get_slice_parallel_world_size(self):
        return self.slice_group_world_size
