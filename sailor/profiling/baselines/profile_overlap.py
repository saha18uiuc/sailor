...
def profile(args):
    ...
    if local_rank == 0:
        ...
        env_config_path = args.coef_file
        ...
        if key not in config:
            config[key] = {"1": 1.0}
        config[key][str(args.world_size)] = overlap_coe
        print('\n********************')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--coef-file", type=str, help='result file')
    parser.add_argument("--gpu-type", type=str, help='GPU')
    parser.add_argument("--world-size", type=int, help='World size')
    parser.add_argument("--master-addr", type=str, help='IP address')
