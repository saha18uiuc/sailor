...
def main(args):
    for i in range(16, 31):
        ...
        cmd = f"python test_collectives.py --rank {args.rank} --master_ip {args.master_ip} --gpus_per_node {args.gpus_per_node} --world_size {args.world_size} --data_size {num_bytes} --num_runs 50 --test {args.test}"
        if i==16:
            cmd += " --add_header"
        os.system(cmd)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test', type=str, default="", help='Kind of test to run')
