...
import sailor.Planner.baselines.Aceso.aceso_prims as aceso_prims
import copy 
import time 
from sailor.Planner.baselines.Aceso.aceso_prims import reset_move_count, get_move_count

def search_init(argString, training_config):
    global args, config_visited
    
    utils_init(training_config)
    cost_model_init(argString)
    prims_init(argString)
    policy_init(argString)
    args = parse_args(argString)

    print_args(args)

    read_profiled_time(args.model_name, args.profiled_time_path)
...
def multi_hop_search(config, hop_index, initial_time, adaptive_flag):
    ...
    actions_all, turn_back_actions, stage_type = get_actions(config, bottleneck, aceso_prims.action_resource_table, adaptive_flag=adaptive_flag)
...
def run_search(num_stages, queue=None):
    ...
    if config is None:
        print(f"no init config for num_stages {num_stages}")
    ...
    if best_config is not None:
        if args.config_save_path is not None:
            dump_config_to_json(best_config, f'{args.config_save_path}{args.model_name}_{best_config.num_stages}stages_{args.config_suffix}.json', args)
        else:
            config_dict = dump_config(best_config, f'{args.model_name}_{best_config.num_stages}stages_{args.config_suffix}', args)
    else:
        print(f"none config for num_stages = {num_stages}")
    ...
    if best_config is not None:
        predict_time_breakdown(best_config, print_time=True, print_memory=True)
    ...
    if queue is not None:
        ...
        if args.config_save_path is not None:
            ...
            result_dict[num_stages] = current_min_time, config_mem, get_explored_cases(), sum(search_time_list), get_hit_resources(), config_dict
        ...
        if args.config_save_path is not None:
            ...
            return current_min_time, config_mem, get_explored_cases(), sum(search_time_list), get_hit_resources(), config_dict
...
if __name__=='__main__':
    ...
    --num-gpus-per-node 4 \
    --memory-limit 12000 \
    --profiled-time-path profiles/ \
    --config-save-path logs/configs/gpt/1_3B/ \
    --max-num-hops 7 \
    --time-budget-total 200 \
    --initial-point balance"""

    search_init(argString)
