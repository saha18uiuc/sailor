...
def run(rank, world_size, data_size_list, model, torch_data_type):
    args = parse_args()
    init_method = 'tcp://'
    ...
    for collective_type in collectives:
        ...
        for data_size in data_size_list:
            ...
            if data_size_in_mb not in avg_time_list:
                print(f"[rank {rank}] {model} profiling {collective_type} ({world_size}GPUs) ({data_size} = {data_size_in_mb} MB)...\n")        
        ...
        if rank == 0:
            ...
            save_file_name = f"prim_{model}_{collective_type}_{world_size}gpus.csv"
...
def run_profile(task):
    ...
    for mbs in mbs_list:
        for tp in tp_size_list:
            for algo in algo_list:
                file_name = args.prof_op_time_path + f"{model}_mbs{mbs}_tp{tp}_algo{algo}.csv"
    ...
    torch.multiprocessing.spawn(run, args=(args.prof_tp_size, data_size_list, model, torch_data_type), nprocs=args.prof_tp_size, join=True)
