...
from megatron.model.flex_opt import FlexOPTModel
...
def get_model(model_name):
    ...
    if model_name in ["OPT-30", "OPT-350"]:
        # num_layers, seq_len, hidden_size, ffn_hidden_size, num_attention_heads, kv_channels, vocab_size = model_prof_configs[model_name]["configs"]
        get_params_dtype()
        ...
        args.padded_vocab_size = vocab_size # padded vocab_size
        args.num_layers = num_layers
        args.seq_length = seq_len
        model = FlexOPTModel(num_layers=num_layers, hidden_size=hidden_size, ffn_hidden_size=ffn_hidden_size, num_attention_heads=num_attention_heads, kv_channels=kv_channels, profiling=True)
    elif model_name in ["GPT-Neo-2.7"]:
        # num_layers, seq_len, hidden_size, ffn_hidden_size, num_attention_heads, kv_channels, vocab_size = model_prof_configs[model_name]["configs"]
        get_params_dtype()
        args.seq_length = seq_len
        args.hidden_size = hidden_size
...
def run_profile(task):
    ...
    op_list = get_model(model).full_op_list
    tp_size = args.prof_tp_size
    algo_list = model_prof_configs[model]["algo"]
    params_dtype = args.params_dtype
    save_filename_prefix = f"{model}"
...
if __name__ == "__main__":
    ...
    model_names = [args.prof_model_name]
    for model in model_names:
        ...
        for mbs in micro_batch_sizes:
            all_prof_tasks.append({"model": model, "mbs": mbs})
