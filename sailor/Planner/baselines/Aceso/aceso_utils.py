...
from sailor.Planner.baselines.Aceso.model_ops_info import get_full_op_list

model_configs = {}

def utils_init(training_config):
    ...
    num_attention_heads = training_config["heads"]
    kv_channels = hidden_size//num_attention_heads
    vocab_size = training_config["vocab_size"]
    model_configs[model_name] = (num_layers, seq_len, hidden_size, ffn_hidden_size, 
                                 num_attention_heads, kv_channels, vocab_size)

# model_configs = {
#     # model_size: (num_layers, seq_len, hidden_size, ffn_hidden_size, num_attention_heads, kv_channels, vocab_size)
#     "GPT-Neo-2.7": (32, 2048, 2560, 2560*4, 32, 2560//32, 128256),
#     "OPT-30": (48, 2048, 7168, 7168*4, 56, 7168//56, 50272),
#     "OPT-350": (24, 2048, 1024, 1024*4, 16, 1024//16, 50272),
# }

## NOTE: fp16 will introduce a "main_param" in Megatron
memory_ratio = {
    "GPT-Neo-2.7": {
        "fp16": {"main_params": 2, "optimizer": 4},
        "fp32": {"main_params": 0, "optimizer": 2}
    },
    "OPT-30": {
        "fp16": {"main_params": 2, "optimizer": 4},
        "fp32": {"main_params": 0, "optimizer": 2}
...
def dump_config_to_json(config, file_name, args):
    ...
    if model_name in ["GPT-Neo-2.7", "OPT-30", "OPT-350"]:
        _, seq_len, hidden_size, ffn_hidden_size, num_attention_heads, kv_channels, vocab_size = model_configs[model_name]
...
def dump_config(config, print_name, args):
    model_name = args.model_name
    num_layers = args.num_layers
    config_dict = {}
    config_dict["model_name"] = model_name
    ...
    print(f"config has been found {print_name}")   
    # config_dicts[print_name] = config_dict 
    return config_dict
...
def parse_args(argString=None):
    ...
    if args.model_name not in ["GPT-Neo-2.7", "OPT-30", "OPT-350"]:
        raise RuntimeError(f"model {args.model_name} is not supported yet.")

    if args.num_layers is None:
        if args.model_name in ["GPT-Neo-2.7", "OPT-30", "OPT-350"]:
            args.num_layers = model_configs[args.model_name][0]
    ...
    if args.start_num_stages is None or args.end_num_stages is None:
        ...
        args.end_num_stages = min(args.num_gpus, 128)
    ...
    if args.fp16:
        args.memory_main_params = memory_ratio[args.model_name]["fp16"]["main_params"]
        args.memory_optimizer = memory_ratio[args.model_name]["fp16"]["optimizer"]
    else:
        args.memory_main_params = memory_ratio[args.model_name]["fp32"]["main_params"]
        args.memory_optimizer = memory_ratio[args.model_name]["fp32"]["optimizer"]
...
def save_and_print_top_configs(result_dict, args):
    ...
    for i in range(args.start_num_stages, args.end_num_stages + 1):
        if i in result_dict and result_dict[i] is not None:
            config_time, config_mem, explored_cases, search_time, case_distribution = result_dict[i][:5]
            if config_time > 0:
    ...
    for i in range(min((args.end_num_stages - args.start_num_stages + 1), args.num_of_saved_configs, len(sorted_time_list))):
        ...
        if args.config_save_path is not None:
            src_file = f'{args.config_save_path}{args.model_name}_{stage_num}stages_{args.config_suffix}.json'
            dst_file = f'{args.config_save_path}top_configs/{args.model_name}_{stage_num}stages_{args.config_suffix}.json'
            if os.path.exists(src_file):
                shutil.copy(src_file, dst_file)
            with open(src_file, 'r') as f:
                opt_dict.append(json.load(f))
        else:
            opt_dict.append(result_dict[stage_num][-1])
        
        config_time, config_mem, explored_cases, search_time, case_distribution = result_dict[stage_num][:5]
...
def print_search_details(config, args, num_stages, num_targets_list, num_hops_list, search_time_list, config_time_list, reserved_mem_list, num_explored_cases):
    ...
    if config is not None:
        ...
        save_config_info_to_csv(config, reserved_mem_list, f'{args.config_save_path}csv/info_{args.model_name}_{config.num_stages}stages_{args.config_suffix}.csv')  
        save_distribution_to_csv(num_targets_list, num_hops_list, f'{args.log_path}trends/distribution_{args.model_name}_{config.num_stages}stages_{args.config_suffix}.csv')       
    else:
    ...
    save_search_trend_in_csv(accum_list, config_time_list, f"{args.log_path}trends/{args.model_name}_{num_stages}stages_init_{args.initial_point}_{args.max_num_hops}hops_{args.config_suffix}.csv")
