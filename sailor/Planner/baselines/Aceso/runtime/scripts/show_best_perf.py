...
file_path = os.path.join(src_data_path, "runtime", model_name, "csv")
    ...
    for file_name in files:
        with open(os.path.join(file_path, file_name), 'r') as f:
            ...
            for row in csv_reader:
                ...
                thpt = global_bs/time
                config_name = os.path.join(src_data_path, "configs", file_name.split("_stage")[0] + ".json")
                if thpt > max_thpt:
                    best_result = [global_bs, time, thpt, config_name]
                    max_thpt = thpt
...
print(f"Batch Size\t Time(s)\t Thpt(samples/s)")
