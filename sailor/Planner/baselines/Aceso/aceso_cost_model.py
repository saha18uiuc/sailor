...
import math
import os 
from sailor.Planner.baselines.Aceso.model_ops_info import get_op_spec, get_op_list, get_no_recompute_op_list
from sailor.Planner.baselines.Aceso.aceso_utils import * 

def cost_model_init(argString):
    global args, op_list, ops_not_recomputed, math_log_2, global_mbs_index
...
def read_profiled_time(model_name, time_path):
    ...
    for mbs in mbs_list:
        for tp in tp_size_list:
            ...
            for algo_index in algo_list:
                ...
                src_data_file = time_path + model_name + f"_mbs{mbs}_tp{tp}_algo{algo_index}.csv"
                try:
    ...
    for prim in prim_list:
        ...
        for num_gpus in comm_num_gpus_list:
            ...
            if model_name == "scale-layer":
                ...
                src_data_file = time_path + f"prim_{model_name}_{prim}_{num_gpus}gpus.csv"
            with open(src_data_file) as f:
                src_data = csv.reader(f)
                line_index = 0
                for row in src_data:
...
def predict_time_breakdown(config, print_time=False, print_memory=False):
    ...
    for i in range(config.num_stages):
        ...
        if print_time:
            ...
            print("Pure Time: [stage {}], {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, ".format(i, fwd_comp/1000, (bwd_comp + recomp_time)/1000, recomp_time/1000, in_comm/1000, out_comm/1000, tp_comm/1000))

        ## compute ideal time of each sta vge
...
def predict_time_breakdown_beta(config, print_time=False, print_memory=False):
    base_batch_size = config.micro_bs
    global_batch_size = config.global_bs
    num_batches = global_batch_size // base_batch_size
    if print_time:
...
if __name__ == "__main__":
    ...
    read_profiled_time(config_dict["model_name"], args.profiled_time_path)
