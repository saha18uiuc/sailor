...
# Script adapted from https://github.com/DachengLi1/AMP/blob/main/src/amp_utils.py
import torch
import os
import subprocess
import json
...
def simulate(rank_maps, partitions, gbs, micro_bs_list, model_config, oth_list, exp_name):
    ...
    for i in range(len(rank_maps)):
        ...
        if not os.path.isdir(dir_path):
            os.mkdir(dir_path)
        rmap_path = os.path.join(dir_path, "rank_map.json")
        ...
        if rank_map is not None:
            ...
            for oth_host in hosts:
                print(
                    f"Transferring rmap to {oth_host} all other hosts:{hosts}")
                subprocess.run(
                    f"scp {rmap_path} ubuntu@{oth_host}:{rmap_path}", shell=True)
        else:
            for oth_host in hosts:
        ...
        if partition is not None:
            ...
            for oth_host in hosts:
                subprocess.run(
                    f"scp {partition_path} ubuntu@{oth_host}:{partition_path}", shell=True)
        else:
            for oth_host in hosts:
                remove_remote(oth_host, partition_path)

        json_path = os.path.join(
            home_path, 'AMP/DeepSpeed/DeepSpeedExamples/Megatron-LM-v1.1.5-3D_parallelism/examples/ds_config.json')

        if model_type == "gpt2":
            script_path = os.path.join(
                home_path, "AMP/DeepSpeed/DeepSpeedExamples/Megatron-LM-v1.1.5-3D_parallelism/examples/ds_pretrain_gpt2_pipe.sh")
            conf = f" {mp} {pp} {config_n} {config_h} {micro_bs} {gas} {exp_name}"
        else:
            assert model_type == "transgan"
            script_path = os.path.join(
                home_path, "AMP/DeepSpeed/DeepSpeedExamples/Megatron-LM-v1.1.5-3D_parallelism/examples/ds_pretrain_transgan_pipe.sh")
            conf = f" {mp} {pp} {config_n} {config_h} {micro_bs} {gas} {exp_name}"
        ...
        for oth_host in hosts:
            subprocess.run(
                f"scp {json_path} ubuntu@{oth_host}:{json_path}", shell=True)
