...
class StagePerformance:
    ...
    def _get_execution_time(self, device_type: str, key: str) -> float:
        #print(self.profile_data[f'DeviceType.{device_type}'].keys())
    ...
    def get_intra_stage_compute_performance(self, strategies: List[Tuple[int, int]], gbs: int, batches: int):
        stage_performance = []
        for stage_id, intra_strategy in zip(range(len(self.plan.device_groups)), strategies):
            ...
            if hetero_device_group:
                ...
                hetero_bs = [bs] * dp_deg #data_load_balancer.partition_data(device_types, intra_strategy, gbs // batches)
                #print(f"hetero_bs is {hetero_bs}")
