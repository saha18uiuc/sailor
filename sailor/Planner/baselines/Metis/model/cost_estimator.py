...
from sailor.Planner.baselines.Metis.model.utils import partition_layers_by_stage
from sailor.Planner.baselines.Metis.model.cluster_bandwidth import HomoClusterBandwidth, HetClusterBandwidth
from sailor.Planner.baselines.Metis.model.load_balancer import DataLoadBalancer
from sailor.Planner.baselines.Metis.search_space.plan import UniformPlan, InterStagePlan
...
class CostEstimator(ABC):
    ...
    def _get_fb_sync_cost(self, device_types: List[str], tp_deg: int, batch_size: int) -> float:
        ...
        for device_type in device_types:
            ...
            #print(f"fb_sync_cost is {fb_sync_cost}")
    ...
    def get_cost(self, plan: UniformPlan, device_type: str) -> Tuple[float, List, bool]:
        ...
        for stage_id in range(len(stage_layer_counts)):
            ...
            if stage_id == (len(stage_layer_counts) - 1):
                ...
                pp_cost += self._get_pp_cost(activation_size, pp_bandwidth) * 1000 # should be in ms
        ...
        dp_cost = self._get_dp_cost(stage_parameters, dp_bandwidth, dp_deg) * 1000
...
class HeteroCostEstimator(CostEstimator):
    def __init__(self, profile_data: Dict, model_config: ModelConfig, model_volume, gpu_cluster: GPUCluster, max_profiled_batch_size: int):
        super().__init__(profile_data, model_config, model_volume, gpu_cluster)
    ...
    def _get_hetero_device_group_execution_time(self, device_types: List[str], intra_strategy: Tuple[int, int],
        ...
        for dp_id, h_mbs in enumerate(hetero_bs):
            ...
            for h_mbs_slice in comb_h_mbs:
                if h_mbs_slice > self.max_profiled_batch_size:
    ...
    def _get_execution_cost(self, device_types: List[str], start_layer_id: int, end_layer_id:int,
                            intra_strategy: Tuple[int, int], gbs: int, batches: int, mbs=0) -> float:
        dp_deg, tp_deg = intra_strategy
        ...
        # heterogeneous device group
            ...
            if mbs>0:
                hetero_bs = [mbs]*intra_strategy[0]
            else:
    ...
    def get_cost(self, plan: InterStagePlan, strategies: List[Tuple[int, int]], layer_partition: List[int],
        ...
        for stage_id, intra_strategy in zip(range(plan.num_stage), strategies):
            ...
            execution_cost = self._get_execution_cost(device_types, start_layer_id, end_layer_id, intra_strategy, plan.gbs, plan.batches, mbs=mbs)
            ...
            if stage_id == (plan.num_stage - 1):
                ...
                pp_cost += self._get_pp_cost(activation_size, pp_bandwidth) * 1000

            #print(f"stage: {stage_id}, execution_cost: {execution_cost}, pp_cost: {pp_cost}, batches is {plan.batches}")
            ...
            dp_costs.append(self._get_dp_cost([stage_parameters], dp_bandwidth, dp_deg) * 1000)
