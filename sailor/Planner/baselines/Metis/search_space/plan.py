...
from sailor.Planner.baselines.Metis.search_space.device_group import gen_device_group_shapes, gen_dgroups_for_stages_with_variance
...
from sailor.Planner.baselines.Metis.utils import DeviceType
...
class InterStagePlanGenerator:
    def __init__(self, device_types: set, num_devices: int, gbs: int, num_layers: int, variance: float = 0.5,
        ...
        print(f"node_sequences is {self.node_sequences}")
        self.num_devices = num_devices
        self.gbs = gbs
        ...
        # print(f"dg is {self.device_groups}, num_devices is {num_devices}, group_shapes is {self.group_shapes}")
        # time.sleep(100)

        if len(self.device_groups) > 0:
