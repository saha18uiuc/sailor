# Code from: https://github.com/DS3Lab/DT-FM/blob/master/scheduler/heuristic_evolutionary_solver/scheduler.py
...
def compute_data_parallel_cost(partition_size, send_gradient_size, peer_delay, peer_bandwidth, candidate_partition=None):
...
def compute_pipeline_parallel_cost(
    ...
    def bipartite_matching(partition_size, peer_delay, send_activation_size, peer_bandwidth, candidate_partition_0, candidate_partition_1):
    ...
    for i in range(way):
        for j in range(i+1, way):
            ...
            cross_partition_cost[i, j], dp_pipeline_parallel_match[i][j] = bipartite_matching(
                partition_size, peer_delay, send_activation_size, peer_bandwidth,
...
def get_pipelines(way, partition_size, num_devices, candidate_partition=None, candidate_pipeline_parallel_path=None,
...
def GCMA(
    ...
    def five_point_crossover(parent1=None, parent2=None):
        ...
        #print(partition_sizes, partition_idx, parent2_str)
    ...
    #print(f"STEP 1: POPULATION IS {candidate_partitions}, partition_size is {partition_size}")

    for i, candidate_partition in enumerate(candidate_partitions):
        candidate_partition = [candidate_partition[i: i + partition_size]
                               for i in range(0, num_devices, partition_size)] # each of the lists inside the list are a stage (a DP group)
        print(len(candidate_partition), len(candidate_partition[0]))
