...
from sailor.Planner.baselines.FlashFlex.src.cost_modeling import MemoryCost, TimeCost

from typing import List
from copy import deepcopy

def get_considered_stragegies(i, not_use_tp):
    """
...
def refine_strategies(strategies, pp_devices, configs):
    ...
    tp_pp_tradeoff(configs)

    return ngpu_strategy


def gen_strategy(recovered_parts, path, configs, not_use_tp):
    ...
    for stage_counts in intra_counts:
        for ngpus in stage_counts:
            strategies.append(get_considered_stragegies(ngpus, not_use_tp))
    ...
    # like [[2, 1], [1], [1]]
    ngpu_strategy = refine_strategies(strategies, flattened_parts, configs)
