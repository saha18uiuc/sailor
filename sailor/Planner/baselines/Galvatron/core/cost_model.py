...
class MemoryCostModel:
    def __init__(self,
        ...
        self.model_states_size = self.parameter_size # we pass it in bytes already
        ...
        chunks = int(chunks)
        #print(f"Global_batch_size is {global_batch_size}, mbsz is {mbsz}, chunks is {chunks}")
...
class TimeCostModel:
    def __init__(self,
        ...
        #print(f"comm_coe_dict is {self.comm_coe_dict}")
        ...
        #print(f"forward_computation_time: {forward_computation_time}, self.bs: {self.bs}, self.tp_size {self.tp_size}, layer_num: {self.layer_num}")
    ...
    def gen_result(self):
        ...
        if self.pp_size >= 1:
            if self.tp_size == 1 and self.dp_size > 1: # pp+dp
                ...
                #print(f"HERE, FCT: {self.fct}, BCT: {self.bct}, TP SIZE: {self.tp_message_size}, TC: {self.tc}, MICROBATCH: {self.microbatch}")
                if self.microbatch == False:
                    result = self.fct + self.bct + self.tp_message_size*self.tc
                else:
...
def pipeline_costmodel(timecostmodel, layer_num_list, timecostmodel_args_list, strategies, partition, chunks, bsz, return_stage_cost=False):
    # print(f"timecostmodel: {timecostmodel}")
    # print(f"layer_num_list: {layer_num_list}")
    # print(f"timecostmodel_args_list: {timecostmodel_args_list}")
    # print(f"strategies: {strategies}, {len(strategies)}")
    ...
    from sailor.Planner.baselines.Galvatron.utils import form_strategy, strategy_str2list
    ...
    # print("------------------------ TIME COSTS ------------------------")
    # print(timecosts_bsz_chunked)
    # print(timecosts_bsz_compute)

    # print("------------------------ STAGE COSTS ------------------------")
    # print(stage_costs_bsz_chunked)
    # print(stage_costs_compute)
