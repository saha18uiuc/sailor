...
from sailor.Planner.baselines.Galvatron.core import MemoryCostModel, TimeCostModel, DpOnModel
from sailor.Planner.baselines.Galvatron.core.cost_model import pipeline_costmodel
...
class GalvatronSearchEngine():
    ...
    def set_cluster_info(self, num_nodes, num_gpus_per_node, memory_constraint):
        self.args.num_nodes = num_nodes
        self.args.num_gpus_per_node = num_gpus_per_node
        self.args.gpu_num = num_nodes * num_gpus_per_node
        self.args.memory_constraint = memory_constraint
    ...
    def time_profiling_path(self):
        ...
        time_config_path = "compute_profile.json"
    ...
    def get_profiled_hardware_configs(self):
        ...
        self.allreduce_bandwidth, self.allreduce_comm_coe = read_allreduce_bandwidth_config(os.path.join(self.path, bandwidth_config_path), gpu_num=args.gpu_num)
        self.p2p_bandwidth, self.p2p_comm_coe = read_p2p_bandwidth_config(os.path.join(self.path, bandwidth_config_path))
        self.overlap_coe = read_json_config(os.path.join(self.path, bandwidth_config_path))['overlap_coe']
    ...
    def parallelism_optimization(self):
        ...
        for bsz in self.BSZs:
            pp_stage_dict = pp_stage_dict_for_bsz[bsz] # split of layers into stages
            results[bsz] = dict()
            ...
            for chunk in chunk_list:
                ...
                for pp in pp_deg_list:
                    ...
                    #print(pp, mbsz_dict[pp], chunk)
                    if (mbsz_dict[pp] not in [1,2,4]):
                        break
                    #time.sleep(1000)
                ...
                res = self.dynamic_programming(bsz, chunk, mbsz_dict, pp_stage_dict)
                res_config = self.convert_results(res, bsz, chunk, pp_stage_dict_for_bsz[bsz])

                results[bsz][chunk] = res
                if res_config:
                    all_results.append(res_config)
        ...
        # config = self.save_results(re, optimal_bsz, optimal_chunk, pp_stage_dict_for_bsz[optimal_bsz])
    ...
    def set_searching_bsz(self):
        ...
        if args.settle_bsz is not None and args.settle_bsz > 0:
            ...
            print(f"[Searching Batch Sizes Info], Settle bsz: {args.settle_bsz}, Min bsz: {self.min_bsz}, Max bsz: {self.max_bsz}")
            return
    ...
    def dynamic_programming(self, bsz, chunk, mbsz_dict, pp_stage_dict):
        args = self.args
        print('---------------------- START DP, bsz=%d'%bsz, pp_stage_dict)
        ...
        throughput = 1 / min_cost # in iterations/sec
    ...
    def convert_results(self, results, bsz, chunk, pp_stage_dict):
        ...
        if re['min_pp_deg'] > 0 and re['min_res_list'] is not None:
            ...
            config['throughput'] = re['throughput']

            return config

    def save_results(self, results, bsz, chunk, pp_stage_dict):
        re, optimal_bsz = results, bsz
        print(f"SAVE_RESULTS, re is {re}, optimal_bsz is {optimal_bsz}")
        args = self.args
        if re['min_pp_deg'] > 0 and re['min_res_list'] is not None:
            result_strategy = []
            if isinstance(re['min_res_list'],list):
                ...
                for tp in total:
                    if pp*tp<=gpu_num:
                        ...
                        if tp==1 or tp == gpu_num/pp:
                            ...
                            strategies.append([pp,tp,dp,{'tp':0,'fsdp':0}]) # what do these stand for?
...
def optimal_chunk_func_default(local_bsz, strategy, microbatch_size):
    # if strategy[0] == 1:
    #     return 1
    # local_bsz is the batch size per stage

    local_bsz = local_bsz // strategy[1] # per DP rank
