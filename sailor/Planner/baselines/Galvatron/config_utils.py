...
from sailor.Planner.baselines.Galvatron.utils import dict_join_dirname
...
def model_name(config, args=None):
    return 'hidden%d_head%d_seqlen%d'%(config["hidden_size"], config["heads"], config["max_position_embeddings"])
