...
void GenSchedule::InitQueues() {
  ...
  //printf("At init queues, num_mini_ is %d\n", num_mini_);
...
TaskQueue* GenSchedule::PickQueue_1f1b(int stage, char* identifier) {
  ...
  //printf("warmup_fwds is %d, stage is %d, num_fwds is %d, num_bwd is %d\n", warmup_fwds, stage, num_fwds_done[stage], num_bwds_done[stage]);
...
TaskQueue* GenSchedule::PickQueue(int stage, char* identifier) {
  ...
  return PickQueue_1f1b(stage, identifier);
}
...
void GenSchedule::Generate(std::vector<schedule_task> sched[]) {
      ...
      switch(queue_ids[i]) {
        ...
        case '0':
          if (!last_stage) {
            ...
            //printf("PUSH, deq size is %d\n", fwd_queues_[i+1]->size());
  ...
  //       printf("%c,%d;", sched[i][j].task, sched[i][j].index);
  //     }
  //   //}
  //   printf("]\n");
  // }
