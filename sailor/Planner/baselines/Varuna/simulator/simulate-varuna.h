...
public:
  Stage(int stage_num, int depth, int num_mb, vector<schedule_task> s)
    ...
    gpipe = false; //gpipe_env==NULL ? 0 : (bool) atoi(gpipe_env);
    char *pd_1f1b_env = getenv("PD_1F1B");
    pd_1f1b = true; //pd_1f1b_env==NULL ? 0 : (bool) atoi(pd_1f1b_env);
    fwd_ = new Queue('f');
    bwd_ = new Queue('b');
    rc_ = new Queue('r');
...
class Simulator {
public:
  Simulator(int depth, int num_mini, float fwd, float bwd, float send, float allr,
             float last_fwd=0.0, float last_bwd=0.0, int send_long=0, int dp=1)
  : pipeline_depth_(depth), num_mini_(num_mini), sendact_time_(send),
    ...
    //printf("At simulator, num_mini_ is %d\n", num_mini_);
  ...
  float fwd_compute_time(int stage) {
    float basetime = fwd_time_[stage];
    float retval = add_jitter(basetime, 0);
  ...
  float bwd_compute_time(int stage) {
    float basetime = bwd_time_[stage];
    float retval = add_jitter(basetime, 0);
    if(stage < pipeline_depth_ - 1){
  ...
  float sendact_time(int s, int d) {
    bool same_vm = is_same_vm(s,d);
    float basetime = same_vm ? sendact_time_ : sendact_long_time_;
    float retval = add_jitter(basetime, 1);
    if( same_vm) {
  ...
  float sendgrad_time(int s, int d) {
    bool same_vm = is_same_vm(s,d);
    float basetime = same_vm ? sendgrad_time_ : sendact_long_time_;
    float retval = add_jitter(basetime, 1);
    if(is_same_vm(s,d)) {
      MAX_SEND = (MAX_SEND > retval) ? MAX_SEND : retval ;
      MIN_SEND = (MIN_SEND < retval) ? MIN_SEND : retval ;
...
private:
  ...
  float sendact_time_, sendgrad_time_, allreduce_time_;
  int sendact_long_time_;
  int GPUS_PER_VM;
  vector<float> fwd_time_, bwd_time_;
  normal_distribution<double> distribution;
  normal_distribution<double> compute_distribution;
  default_random_engine net_generator; //, comp_generator;

  float MAX_SEND = 0.0, MIN_SEND = 10000000.0;
  float MAX_LONG_SEND = 0.0, MIN_LONG_SEND = 10000000.0;

  float MAX_FWD = 0.0, MIN_FWD = 10000000.0;
  float MAX_BWD = 0.0, MIN_BWD = 10000000.0;
  float MAX_FWD_LONG = 0.0, MIN_FWD_LONG = 10000000.0;
  float MAX_BWD_LONG = 0.0, MIN_BWD_LONG = 10000000.0;
