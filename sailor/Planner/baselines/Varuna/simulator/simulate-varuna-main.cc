...
int main(int argc, char** argv) {
  ...
  float send_time = atoi(argv[arg_index++]);
  float allreduce_time = atoi(argv[arg_index++]);
  // int last_fwd_time = 0,last_bwd_time = 0;
  // if(argc > arg_index){
  //   last_fwd_time = atoi(argv[arg_index++]);
  //   last_bwd_time = atoi(argv[arg_index++]);
  // }
  float send_long_time = (argc > arg_index) ? atoi(argv[arg_index++]) : 0;
  ...
  Simulator s(pipeline_depth, num_mini, 0.0,0.0, send_time, allreduce_time, 0,0, send_long_time, dp);
