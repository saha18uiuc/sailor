...
Queue* Stage::PickNextComputeQueue(int64 now) {
  ...
  //printf("********************* Stage %d, curr_task_ind %d, t.task is %c\n", stage_num_, curr_task_ind, t.task);
  ...
  if (t.task == '2' && (q = QueueReady(bwd_, now)) ) {
      //printf("HERE\n");
      if (q->front().micro_batch == t.index ) {
...
void Stage::ServiceQueues(Simulator* sim, int64 now) {
  ...
  //printf("Inside service queue, gpu_busy_ is %d, network_busy_ is %d\n", gpu_busy_, network_busy_);
  ...
  if (!gpu_busy_){
    if(q = PickNextComputeQueue(now)) {
      ...
      q->pop_front();
      //printf("------------------ Stage %d, add event %d\n", stage_num_, IdToState(q->id));
...
bool Simulator::AddEvent(int64 scheduled_time, Event e) {
  //printf("Add event stage: %d, mb_num: %d, state: %d\n", e.stage, e.mb_num, e.state);
...
bool Simulator::PopNextEvent(Event* event) {
  //printf("AT POP, EVENT LIST HAS SIZE %d\n", event_list_.size());
...
void Simulator::DumpState() {
  ...
  printf("\nCompute times (fwd = %f, bwd = %f)", fwd_time_[0], bwd_time_[0]);
  printf("\nLast stage compute times (fwd = %f, bwd = %f)", fwd_time_[pipeline_depth_-1], bwd_time_[pipeline_depth_-1]);
  printf("\nCommunication times (activation = %f, grad = %f)", sendact_time_, sendgrad_time_);
  printf("\nLong comm times (activation/grad = %f)", sendact_long_time_);
  printf("\n");
}
...
void Simulator::Simulate() {
  ...
  if (!stages_[0]->isDone()){
    printf("End of simulation:  Mini-batch time (usec) = 0\n");
    return;
  }
  #ifdef DEBUG
  for(int i = 0; i < pipeline_depth_; i++){
  ...
  #endif
  printf("End of simulation:  Mini-batch time (usec) = %f\n", clock_now_micros_ + allreduce_time_);
  printf("Min send: %f, max send %f\n", MIN_SEND, MAX_SEND);
  printf("Min long send: %f, max long send %f\n", MIN_LONG_SEND, MAX_LONG_SEND);
  printf("Min fwd: %f, max fwd %f; min bwd %f, max bwd %f\n", MIN_FWD, MAX_FWD, MIN_BWD, MAX_BWD);
  printf("Min long fwd: %f, max long fwd %f; min long bwd %f, max long bwd %f\n", MIN_FWD_LONG, MAX_FWD_LONG, MIN_BWD_LONG, MAX_BWD_LONG );
}