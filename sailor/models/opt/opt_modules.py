...
class GPTEmbeddings(nn.Module):
    def __init__(self, config, res, ind):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        if dtype_attn_weights == torch.float16:
            attn_weights = nn.functional.softmax(
                attn_weights, dim=-1, dtype=torch.float32).to(dtype_attn_weights)
        else:
...
class GPTBlock(OPTDecoderLayer):
    def __init__(self, config, *args, use_checkpoint=False, **kargs):
        ...
        def mlp_res(hidden_states: torch.Tensor) -> torch.Tensor:
            ...
            hidden_states = (
                residual + hidden_states).view(hidden_states_shape)
            return hidden_states
...
'''
def get_opt_causal_lm_specs(config, res={}, grad_ckpt=False,
    ...
    # for k,v in sd.items():
    #     num_params += v.numel()
    # print(f"EMBEDDING: {num_params} parameters")
    specs.append(emb)

    for i in range(1, config.num_hidden_layers + 1):
        #l = GPTBlock(config, res=res, ind=i)
        l1 = LayerSpec(GPTBlock, config, res=res, ind=i)
        # sd = l.state_dict()
        # num_params = 0
        # for k,v in sd.items():
        #     num_params += v.numel()
        # print(f"Transformer {i}: {num_params} parameters")
        specs.append(l1)
    ...
    # for k,v in sd.items():
    #     num_params += v.numel()
    # print(f"HEAD: {num_params} parameters")
    specs.append(head)
    return specs
'''