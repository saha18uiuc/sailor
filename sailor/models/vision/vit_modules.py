...
# Copied from Torchvision with some modifications
...
class CustomConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, patch_size, image_size, hidden_dim):
        super().__init__()
...
class CustomEncoderLast(nn.Module):
    def __init__(
        self,
        hidden_dim: int,
        norm_layer: Callable[..., torch.nn.Module] = partial(nn.LayerNorm, eps=1e-6)
...
class Encoder(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        input = input + self.pos_embedding
        x = self.ln(self.layers(self.dropout(input)))
        # Classifier "token" as used by standard language architectures
        x = x[:, 0]
        return x
...
class CustomVisionTransformer(nn.Module):
    def __init__(
        ...
        self.conv_proj = CustomConv(
            in_channels=3, out_channels=hidden_dim, kernel_size=patch_size, stride=patch_size,
            patch_size=patch_size, image_size=image_size, hidden_dim=hidden_dim
        )
        ...
        self.custom_dropout = CustomDropout(seq_length, hidden_dim, dropout)
        ...
        self.custom_encoder_last = CustomEncoderLast(hidden_dim, norm_layer)
        ...
        if isinstance(self.conv_proj.conv, nn.Conv2d):
            # Init the patchify stem
            fan_in = self.conv_proj.conv.in_channels * \
                self.conv_proj.conv.kernel_size[0] * self.conv_proj.conv.kernel_size[1]
            nn.init.trunc_normal_(self.conv_proj.conv.weight,
                                  std=math.sqrt(1 / fan_in))
            if self.conv_proj.conv.bias is not None:
                nn.init.zeros_(self.conv_proj.conv.bias)
...
def _vision_transformer(
    ...
    **kwargs: Any,
    ...
    if weights is not None:
        ...
        _ovewrite_named_param(kwargs, "image_size",
                              weights.meta["min_size"][0])
...
def vit_h_14(*, weights: Optional[ViT_H_14_Weights] = None, progress: bool = True, **kwargs: Any) -> CustomVisionTransformer:
    """
